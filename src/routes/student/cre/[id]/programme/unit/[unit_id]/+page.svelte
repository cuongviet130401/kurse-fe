<script>
  ...
  import { ProgressRadial, tocCrawler, TableOfContents } from '@skeletonlabs/skeleton';
  ...
  let y;
	...
	afterUpdate(async () => {
		...
		let headings = Array.from(renderedContentElement.querySelectorAll("h1, h2, h3, h4"));
		let headingPositions = headings.map(item => item.getBoundingClientRect().top);

		for (let i = 0; i < headingPositions.length; i++) {
			if (headingPositions[i] >= 200 && headingPositions[i] <= 250) {
				// 1. reset active state for all components
				let tocrefDivs = Array.from(renderedTocElement.querySelectorAll("div.tocref"));
	...
	<div class="flex w-full align-center justify-center items-center">
		...
		<div class="space-x-2 grow-0">
			<div class="flex justify-center align-center items-center h-full">
				<a href={`/student/cre/${courseId}/programme/unit/${unitId-1}`} target="_self" class="btn bg-transparent">
					<i class="bi bi-chevron-left mr-2"></i>
					<span>Prev Unit</span>
				</a>
				<a href={`/student/cre/${courseId}/programme/unit/${unitId+1}`} target="_self" class="btn variant-filled-primary text-white">
					<span>Next Unit</span>
...
<section class="w-full p-4 m-4 relative" >
	...
	<div class="w-full flex relative">
		...
		<main class="hidden" bind:this={unitContentElement} use:tocCrawler={{ mode: 'generate', scrollTarget: '#page' }}>
